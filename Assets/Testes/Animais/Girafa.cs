...
public class Girafa : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (vaicomer)
        {
            if ((Vector3.Distance(this.gameObject.transform.position, destino) < 3f))
            {
    ...
    public Vector3 RandomNavMeshGenerator(float raioCaminhada)
    {
        ...
        UnityEngine.AI.NavMesh.SamplePosition(randomDirection, out hit, raioCaminhada, 1);

        if (UnityEngine.AI.NavMesh.SamplePosition(randomDirection, out hit, raioCaminhada, 1))
        {
